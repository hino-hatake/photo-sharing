...
exports.login = async (req, res) => {
  ...
  if (!login_name) return res.status(400).json({ error: 'login_name is required' });
  try {
    const user = await User.findOne({ login_name }).select('_id first_name last_name').lean();
    if (!user) return res.status(400).json({ error: 'Invalid login_name' });
